...
void BK4819_TuneTo(uint32_t freq, bool precise) {
  ...
  // Без паузы между этими двумя записями калибровка VCO не успевает
  // отработать как настоящий импульс (см. тот же паттерн с явной паузой в
  // BK4819_SetScanFrequency чуть ниже) — бит то опускается, то тут же
  // поднимается обратно почти без задержки на шине SPI, и рекалибровка
  // проходит не каждый раз. На практике это давало приёмник, изредка (не)
  // попадающий на нужную частоту при перестройке скана, хотя RSSI/noise
  // /glitch с большим запасом соответствовали сигналу, когда попадание
  // всё же случалось.
  SYSTICK_DelayUs(300);
...
void BK4819_TurnsOffTones_TurnsOnRX(void) {
  ...
  BK4819_SetAF(BK4819_AF_MUTE);
  BK4819_ExitTxMute();
  // ldo_rf_vsel (REG_37 бит 9): по референсному драйверу должен быть 1 на
  // приёме и 0 на передаче (TxOn_Beep пишет тот же 0x9D1F с этим битом
  // сброшенным). Ничего не возвращало его в RX-состояние при завершении
  // TX — приёмник оставался на TX-напряжении RF LDO до следующей полной
  // переинициализации, что может ощутимо снижать чувствительность.
