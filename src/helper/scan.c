...
//
// Почему именно опрос, а не фиксированная пауза: Noise/Glitch — это не
...
// уверенный результат из шума. Единственная ФИКСИРОВАННАЯ пауза (пробовали
// 5мс, 10мс, потом вручную нашли, что и 9.9мс не хватает части слабых
// сигналов) — это неразрешимый компромисс: либо режем сильные сигналы
// коротким временем, либо теряем скорость на каждом кандидате длинным.
//
// Решение — опрашивать аппаратный бит шумодава (BK4819_IsSquelchOpen)
// каждые CHECK_POLL_US, начиная сразу после FAST_DELAY, и подтверждать
// СРАЗУ как только бит взвёлся:
//  - сильный сигнал обычно взводит бит за первые 1-2 опроса — быстрее,
//    чем старое фиксированное ожидание;
//  - слабый сигнал получает столько времени, сколько нужно интегратору,
//    вплоть до CHECK_MAX_US, а не обрезается на произвольной цифре;
//  - если RSSI успел упасть обратно к полке раньше, чем бит взвёлся —
//    это был случайный ВЧ-пшик/хвост, дальше ждать бессмысленно, выходим
//    сразу, не тратя оставшийся бюджет.
...
static void HandleStateChecking(void) {
  ...
  int32_t threshold = floorLevel + GetMargin();
  ...
  while (elapsed < CHECK_MAX_US) {
    ...
    if (RADIO_GetRSSI(ctx) <= threshold)
      break;
  }
